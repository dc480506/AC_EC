...
if ($_POST['pageView'] == "true") {
 $logger->coursesRecordsViewed($_SESSION['email'], "Current");
}
...
$sel = mysqli_query($conn, "select count(*) as totalcount from course c WHERE c.course_type_id = '$course_type_id' 
                           AND c.program= '$program' AND c.currently_active=1 ");
...
INNER JOIN department d ON c.cid=afd.cid AND c.sem=afd.sem AND c.year=afd.year AND c.course_type_id=afd.course_type_id AND afd.dept_id=d.dept_id WHERE  c.course_type_id = '$course_type_id' AND c.program= '$program' AND c.currently_active=1 "
...
$sql = "select c.cname,c.cid,c.sem,
         ...
         GROUP BY 'all') as app 
         $role_restriction
      from course c WHERE c.course_type_id = '$course_type_id' AND c.program= '$program' AND c.currently_active=1 "
...
while ($row = mysqli_fetch_assoc($courseRecords)) {
   ...
   $data[] = array(
      ...
      "select-cbox" => '<div class="custom-control custom-checkbox">
                        <input ' . $disabled . ' type="checkbox" class="custom-control-input selectrow_current" id="selectrow_current' . $count . '">
