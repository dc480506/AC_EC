...
insert = """INSERT into student_log(email_id,rollno,fname,mname,lname,year_of_admission,dept_id,timestamp,adding_email_id,program) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,'""" + \
...
try:
    for x in range(1, data.nrows):
        ...
        values = (email, rollno, fname, mname, lname,
                  year_of_admission, dept, ts, added_by)
        # executing query
        try:
