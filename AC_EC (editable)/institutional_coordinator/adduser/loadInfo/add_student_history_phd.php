...
$sel = mysqli_query($conn, "select count(*) as totalcount from student_log WHERE program='PHD'");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['totalcount'];

## Total number of record with filtering
$sel = mysqli_query($conn, "select count(*) as totalcountfilters from student_log s INNER JOIN department d ON s.dept_id=d.dept_id WHERE s.program='PHD' and " . $searchQuery . "&& (" . $filterQuery . ")");
$records = mysqli_fetch_assoc($sel);
...
$sql = "select email_id,rollno,dept_name,CONCAT(fname,' ',mname,' ',lname) as name,year_of_admission  
       from student_log s INNER JOIN department d ON s.dept_id=d.dept_id WHERE s.program='PHD' and "
