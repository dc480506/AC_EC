...
$sel = mysqli_query($conn, "select count(*) as totalcount from student WHERE program='PG' $role_restriction");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['totalcount'];

## Total number of record with filtering
$sel = mysqli_query($conn, "select count(*) as totalcountfilters from student s INNER JOIN department d ON s.dept_id=d.dept_id WHERE s.program='PG' $role_restriction and " . $searchQuery . "&& (" . $filterQuery . ")");
$records = mysqli_fetch_assoc($sel);
...
$sql = "select email_id,rollno,current_sem,dept_name,CONCAT(fname,' ',mname,' ',lname) as name,year_of_admission  
       from student s INNER JOIN department d ON s.dept_id=d.dept_id WHERE s.program='PG' $role_restriction and "
