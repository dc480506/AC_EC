...
student_preference_query = "SELECT s.email_id,s.rollno,s.timestamp,m.gpa,stud.dept_id,"+preferences+" FROM " + \
    argument[mapper['student_pref_table']]+" as s inner join student_marks as m on s.email_id=m.email_id and m.sem=s.sem-2 inner join student as stud on stud.email_id=m.email_id WHERE m.program='"+argument[14]+"' and s.sem='" + \
...
for k, v in courses.items():
    ...
    print(str(len(stu_course[k]))+" are there in course " +
          k+" whose max is "+str(v[1])+" and min is "+str(v[0]))
    print()
print("<br>&&&&&&&&&&&&&&&&&&&&&&&&", under, "&&&&&&&&&&&&&&&&&&<br>")
...
while (len(underflow_stu_list) != len(rem_underflow_stu_list)):
    for s in underflow_stu_list:
        ...
        if(s[3] == len(prefl)):
            print(email + " "+str(s[3])+" " +
                  str(len(prefl))+" pref list exhuasted")
            ...
            if(i == len(prefl)):
                ...
                if(next_pref in under):
                    ...
                    if(check_upper_limit(stu_course, courses, next_pref)):
                        stu_course[next_pref].append(
                            [email, 1, time, i+1, prefl, k, curr_marks, dept_id])
                        ...
                        print('Curr Student: '+email+' got course ' +
                              prefl[i]+" as the course was not overflowed")
                        break
                    else:
                        ...
                        if(last_stu[-2] < curr_marks):
                            print(
                                str(next_pref) + " course is overflowed. so curr_stu displace "+str(last_stu[0]))
                            print("curr_stu marks "+str(curr_marks) +
                                  " displace stu marks"+str(last_stu[-1]))
                            ...
                            underflow_stu_list.sort(
                                key=lambda underflow_stu_list: underflow_stu_list[-3])
                            stu_course[prefl[i]].append(
                                [email, 1, time, i+1, prefl, k, curr_marks, dept_id])
                            temp_list = stu_course[prefl[i]]
                            temp_list.sort(key=lambda temp_list: temp_list[-3])
                            stu_course[prefl[i]] = temp_list
                            student_pref_no[email] = i+1
                            print('Curr Student:'+email +
                                  ' got course '+prefl[i])
                            break
                        elif(last_stu[-2] == curr_marks):
                            print("curr_stu marks "+str(curr_marks) +
                                  " same as last stu marks"+str(last_stu[-1]))
                            if(last_stu[2] > time):
                                # print(str((stu_course[next_pref]))+ "is overflowed. so curr_stu displace "+str(last_stu[0]))
                                print("curr_stu time "+str(time) +
                                      " displace stu time"+str(last_stu[2]))
                                stu_course[next_pref].pop()
                                temp = stu_course[prefl[s[3]-1]]
                                temp = [k for k in temp if k[0] != email]
                                stu_course[prefl[s[3]-1]] = temp
                                ...
                                temp_list = stu_course[prefl[i]]
                                temp_list.sort(
                                    key=lambda temp_list: temp_list[-3])
                                stu_course[prefl[i]] = temp_list
                                student_pref_no[email] = i+1
                                print('Curr Student:'+email +
                                      ' got course '+prefl[i])
...
for course, student in stu_course.items():
    ...
    for i in range(len(student)):
        # try:
        query = "INSERT INTO `"+argument[mapper['student_course_table']] + \
            "`(`email_id`, `cid`, `sem`, `year`) VALUES ('" + \
            student[i][0]+"','"+course+"',"+argument[mapper['sem']] + \
                ",'"+argument[mapper['year']]+"')"
        mycursor.execute(query)
        # mydb.commit()
        query = "UPDATE `"+argument[mapper['student_pref_table']]+"` SET allocate_status=1 where email_id='" + \
            student[i][0]+"' and year='"+argument[mapper['year']] + \
                "' and sem="+argument[mapper['sem']]
        # print(query)
        mycursor.execute(query)
        query = "INSERT INTO `"+argument[mapper['pref_student_alloted_table']] + \
            "` (email_id,pref_no) VALUES('" + \
                student[i][0]+"','"+str(student_pref_no[student[i][0]])+"')"
        mycursor.execute(query)
        # except:
        # 	pass
for k, v in dict1.items():
    query = "INSERT INTO `"+argument[mapper['pref_percent_table']]+"`(`pref_no`,`no_of_stu`,`percent`) VALUES('"+str(
        k)+"','"+str(v)+"','"+str(round(v/total_responses, 4)*100)+"')"
    mycursor.execute(query)
# for k,v in student_pref_no.items():
# 	if v!=-1:
